...
/**
 * Creates a Model from the kaggle training dataset using Spark MLlib's Naive Bayes classifier.
 */
...
object NaiveBayesModelCreator {
  ...
  def main(args: Array[String]) {
    ...
    val stopWords = sc.broadcast(StopwordsLoader.loadStopWords(PropertiesLoader.stopWords))
    createAndSaveNBModel(sc, stopWords)
    validateAccuracyOfNBModel(sc, stopWords)
  }
   ...
   * @return
   */
  def removeNewLines(tweetText: String): String = {
  ...
  def createAndSaveNBModel(sc: SparkContext, stopWordsList: Broadcast[List[String]]): Unit = {
    ...
    val labeledRDD = tweetsDF.select("polarity", "status").rdd.map {
      case Row(polarity: Int, tweet: String) =>
        val tweetInWords: Seq[String] = SentimentAnalyzer.retrieveTextForTwitterFeed(tweet, stopWordsList.value)
  ...
  def validateAccuracyOfNBModel(sc: SparkContext, stopWordsList: Broadcast[List[String]]): Unit = {
    ...
    val actualVsPredictionRDD = tweetsDF.select("polarity", "status").rdd.map {
      case Row(polarity: Int, tweet: String) =>
        val tweetText = removeNewLines(tweet)
        val tweetInWords: Seq[String] = SentimentAnalyzer.retrieveTextForTwitterFeed(tweetText, stopWordsList.value)
        (
          polarity.toDouble,
